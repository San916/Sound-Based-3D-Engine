...
// MODIFIES: swap_chain_image_views
// EFFECTS: Creates a swap chain image view for each swap chain image
void create_swap_chain_image_views(
    ...
    for (size_t i = 0; i < swap_chain_images.size(); i++) {
        ...
        if (vkCreateImageView(logical_device, &create_info, nullptr, &swap_chain_image_views[i]) != VK_SUCCESS) {
            throw std::runtime_error("create_swap_chain_image_views(): Failed to create image views!");
        }
    }
}